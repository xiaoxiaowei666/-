...
class Game:
    def __init__(self) -> None:
        ...
        self.highlight_cards = None      # 进贡/还贡阶段需要高亮的牌集合
    ...
    def waitInput(self):
        ...
        # 贡/还贡阶段的特殊处理（高亮允许的牌）
        if self.stage in ('tribute', 'back'):
            # 从 legalActions 中提取允许的牌
            allowed_cards = set()
            card_to_action = {}
            for act_idx, act in enumerate(self.legalActions):
                if act[0] != 'PASS':   # 有效的贡/还贡动作
                    # 动作格式示例：['Single', '2', ['D2']] 或 ['Single', '2', 'D2']
                    card_info = act[2]
