...
class ExampleClient:
    ...
    def on_message(self, ws, message):
        ...
        # 修复 msg 字典本身，确保传给 rule_parse 的数据也是列表
        if "curAction" in msg and isinstance(msg["curAction"], str):
            try:
                msg["curAction"] = ast.literal_eval(msg["curAction"])
            except (SyntaxError, ValueError):
                pass

        if "greaterAction" in msg and isinstance(msg["greaterAction"], str):
            try:
                msg["greaterAction"] = ast.literal_eval(msg["greaterAction"])
            except (SyntaxError, ValueError):
                pass
