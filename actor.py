...
# parser.add_argument('--epsilon', type=float, default=0.2) #h后期需要修改！
#
#
# class Player():
...
#
#         # 数据存储：mb_* 存储单步数据，all_mb_* 存储整局汇总数据
...
#         process_reward = 0.0
#
#         # 获取当前手牌数量（假设 state 中包含 'handcards' 字段）
...
#         if cards_reduced > 0:
#             process_reward += 0.02 * cards_reduced
#
#         self.last_handcards = current_handcards
...
#         """
#         将本局单步数据汇总到 all_mb_* 中，并清空 mb_* 列表。
#         （原 save_data 方法的职责，现独立出来）
#         """
...
#         """
#         final_result: 字符串，如 'y3' 表示赢且基础奖励幅度为3，'n2' 表示输且惩罚幅度为2
#         """
#         # ---- 关键修复：在发送前汇总本局数据 ----
...
#
#         # 可选：添加调试打印，确认数据形状
#         # print(f"[Actor-{self.args.client_index}] Sending: states={states_no_action.shape}, actions={actions.shape}, q={q.shape}, rewards={rewards.shape}")
#
...
#         self.all_mb_q.clear()
#         # mb_* 已在 _aggregate_episode_data 中清空，此处可确保彻底重置
#
#
